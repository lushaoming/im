...
class RandName
{
     ...
     * @param string $surnameSelect
     * @param string $wordSelect
     * @return string
     */
    public static function createRandomName($surname = '', $nameLength = 2, $surnameSelect = '', $wordSelect = '')
    {
        if ($surname != '' && $surname != 'single' && $surname != 'double') return '';
        if ($nameLength < 1 || $nameLength > 2) return '';
        $singleSurnameLength = count(self::$singleSurname);
        $doubleSurnameLength = count(self::$doubleSurName);
        $wordLength = count(self::$word);

        $nameStr = '';
        for ($i = 0; $i < $nameLength; $i++) {
            $nameRandNumber = mt_rand(0, $wordLength - 1);
            $nameStr .= self::$word[$nameRandNumber];
        }
        if ($surname == 'single') {
            $surnameStr = self::$singleSurname[mt_rand(0, $singleSurnameLength - 1)];
        } elseif ($surname == 'double') {
            $surnameStr = self::$doubleSurName[mt_rand(0, $doubleSurnameLength - 1)];
        } else {
            $typeRandNumber = mt_rand(0, 1);
            $surnameStr = $typeRandNumber ? self::$doubleSurName[mt_rand(0, $doubleSurnameLength - 1)] : self::$singleSurname[mt_rand(0, $singleSurnameLength - 1)];;
        }
